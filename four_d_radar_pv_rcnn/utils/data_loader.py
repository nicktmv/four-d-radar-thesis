...
def main():
    ...
    train_indices_path = '../../data/view_of_delft_PUBLIC/lidar/ImageSets/train.txt'
    test_indices_path = '../../data/view_of_delft_PUBLIC/lidar/ImageSets/test.txt'

    print("Loading training data...")
    train_data, train_labels = load_dataset(data_path, label_path, train_indices_path, load_labels_flag=True)
    if train_data:
        print(f"Number of points in the first training file: {len(train_data[0])}")

    print("\nLoading testing data...")
    test_data, test_labels = load_dataset(data_path, label_path, test_indices_path, load_labels_flag=False)
    if test_data:
